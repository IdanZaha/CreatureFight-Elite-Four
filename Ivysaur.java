...
public class Ivysaur extends Creature
{
    ...
    public void attack(int idx)
    {
        ...
        else
        {
            ...
            if(enemyType.equalsIgnoreCase("Electric"))
            {
                 ...
                 */
                getWorld().showText("the attack was not very affective!",getWorld().getWidth()/2,getWorld().getHeight()/2 +26);
            ...
            else if(enemyType.equalsIgnoreCase("Flying"))
            {
                ...
                enemy.getHealthBar().add(-30);
                getWorld().showText("The attack was not affective!",getWorld().getWidth()/2,getWorld().getHeight()/2+26);
                Greenfoot.delay(30);
            }
            else if(enemyType.equalsIgnoreCase("Water"))
            {
                enemy.getHealthBar().add(-60*2);
                getWorld().showText("The attack was VERY AFFECTIVE!",getWorld().getWidth()/2,getWorld().getHeight()/2+26);
            }
            else
            {
                enemy.getHealthBar().add(-120);
                getWorld().showText("The attack was very affective!",getWorld().getWidth()/2,getWorld().getHeight()/2+26);
                Greenfoot.delay(30);
            }
        }
     ...
     */
        /**
     * attackAnimation is the method used for when an attack is used the creature will move
     * up and then down after the attack.
     * 
     ...
     */
        /**
     * switchCreature is the method used for if the creature has fainted it says to choose another
     * creature because it is unable to choose that one.
     * 
     * @params - There is one integer parameter that is used to indentify idx.
     * @returns - There are no return types.
     */
    public void switchCreature(int idx)
    {
        ...
        if(getHealthBar().getCurrent() <= 0)
        {
            JOptionPane.showInputDialog(null, "The creature you have chosen to switch to has fainted, please chose another player.");
        }
        else
        {
            while(getX() > 0)
     ...
     */
        /**
     * switchedIn is used for when a creature is switched in the current goes transparent and
     * moves so the creature can be switched.
     * 
     * @params - There are no parameters.
     * @returns - There are no return types.
     */
